...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty(trim($_POST['customerAddress']))) {
        $customerAddress_err = 'Please Enter the Address of the Customer.';
    } else {
    ...
    foreach ($_POST['productName'] as $x => $value) {
        if ($value == 'null') {
            $productName_err = 'Empty Row!!! Please Select a Product or Delete a Row.';
        } else {
    ...
    if (empty($orderDate_err) && empty($customerName_err) && empty($customerNumber_err) && empty($customerAddress_err) && empty($discount_err) && empty($paid_err) && empty($payOption_err) && empty($method_err) && empty($productName_err))
    {

        $addOrderSql = 'INSERT INTO orders (orderDate, customerName, customerNumber, customerAddress, totalAmount, discount, grandTotal, paidAmount, dueAmount, changeAmount, paymentOption, paymentStatus, shippingMethod, employeeID) 
                    VALUES (:orderDate, :customerName, :customerNumber, :customerAddress, :subTotal, :discount, :grandTotal, :paid, :due, :change, :payType, :payStatus, :method, :employee)';
        ...
        $addOrderStmt->bindParam(':customerNumber', $customerNumber, PDO::PARAM_STR);
        $addOrderStmt->bindParam(':customerAddress', $customerAddress, PDO::PARAM_STR);
        ...
        if ($orderSuccess) {
            ...
            $orderSql = 'INSERT INTO orderHistory (orderDate, grandTotal) 
            VALUES (:orderDate, :grandTotal)';
            $orderStmt = $pdo->prepare($orderSql);
            $orderStmt->bindParam(':orderDate', $orderDate, PDO::PARAM_STR);
            $orderStmt->bindParam(':grandTotal', $grandTotal, PDO::PARAM_STR);
            $orderStmt->execute();
            ...
            foreach ($_POST['productName'] as $x => $productName) {
                ...
                if($totalProductStocks <= 0){
                    $totalStocks = 0;
                }else{
                    $totalStocks = $totalProductStocks;
                }
                
                $productStatus = ($totalStocks >= 1) ? "available" : "unavailable";
        
                $updateProductsStocksSql = 'UPDATE products SET stocks = :totalStocks, status = :productStatus WHERE productID = :productName';
                $updateProductsStocksStmt = $pdo->prepare($updateProductsStocksSql);
                $updateProductsStocksStmt->bindParam(':totalStocks', $totalStocks, PDO::PARAM_INT);
                        ...
                        <div class="mb-2">
                            <div class="input-group">
                                ...
                                <input type="number"
                                    ...
                                    id="customerNumber" name="customerNumber" maxlength="11">
                                <span class="invalid-feedback text-center"><?php echo $customerNumber_err; ?></span>
                            </div>
                        </div> 
                        <div class="mb-2">
                            <div class="input-group">
                                <span class="input-group-text" id="basic-addon3">Customer Address:</span>
                                <input type="text"
                                    class="form-control <?php echo (!empty($customerAddress_err)) ? 'is-invalid' : ''; ?>"
                                    id="customerAddress" name="customerAddress">
                                <span class="invalid-feedback text-center"><?php echo $customerAddress_err; ?></span>
                            </div>
                        </div> 
                                            ...
                                            <tr id="row<?php echo $x; ?>" class="<?php echo $arrayNumber; ?>">
                                                <td>
                                                    <div class="form-group">
                                                        ...
                                                        <span
                                                            class="invalid-feedback text-center"><?php echo $productName_err; ?></span>
                                                    </div>
                                                </td>
                                                <td>
